"""Legacy ANSIM/Grist workspace validator; not a current release gate."""
...
if DB.exists():
    ...
    if check!='ok':
        errors.append(f'sqlite integrity: {check}')
    ...
    for t in ['Documents','Clauses','Visuals','ExtractedTables','Rules','Cases','SourceElements']:
        if t not in names:
            errors.append(f'missing Grist table: {t}')
    ...
    if att != files:
        errors.append(
            f'attachment/file id mismatch: attachments={len(att)}, files={len(files)}'
        )
    for table,col in [
        ('Documents','SourcePDF'),
        ('Visuals','Image'),
        ...
        for rid,raw in cur.execute(
            ...
            try:
                ...
                for aid in ids:
                    if aid not in att:
                        errors.append(f'broken attachment: {table}.{col} row {rid} -> {aid}')
    ...
    for table,col in [
        ...
        if not tr:
            continue
        ref=cur.execute(
            'SELECT displayCol,visibleCol FROM _grist_Tables_column '
        ...
        helper=(
            cur.execute(
                'SELECT parentId,colId,isFormula,formula FROM _grist_Tables_column '
                'WHERE id=?',
                (ref[0],),
            ).fetchone()
            if ref[0]
        ...
        if not ref[1]:
            errors.append(f'missing reference visibleCol: {table}.{col}')

    counts={
        t:cur.execute(f'SELECT COUNT(*) FROM {t}').fetchone()[0]
        for t in ['Documents','Clauses','Visuals','ExtractedTables','SourceElements']
    }
    con.close()
else:
    counts={}
...
if manifest.exists():
    ...
    for r in rows:
        p=ROOT/r['crop_path']
        if not p.exists():
            errors.append(f'manifest crop missing: {r["crop_path"]}')
else:
