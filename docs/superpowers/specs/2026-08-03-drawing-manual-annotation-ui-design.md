...
The implementation reuses the current authoritative backend without defining parallel contracts:

- `contracts.drawing.CoordinateSystem`, `Geometry`, `DrawingCandidate`, and `DrawingConfirmation`
...
The server maps browser actions to existing backend functions. It returns the newly created candidate and confirmation manifest entries. It does not rewrite the case manifest or independently promote any input to Math or Rule Engine authority.
...
verified source/page metadata + candidates
  ...
  -> new manifest entries returned to the caller
  -> caller regenerates the next projection or updates its authoritative manifest
```

The static server response is not an authoritative live case index. A newly created annotation is persisted and returned immediately, while a complete refreshed projection is generated by the caller from verified manifest entries.
...
- Host, Origin, token, content-type, body-size, and case-root rejection;
- browser action through confirmed input and engine binding;
...
Completion also requires repository-wide pytest, Ruff, strict mypy, compileall, Python 3.11/3.13 wheel verification, Windows and Ubuntu validation, and recorded browser QA against an exact commit. GitHub Actions runs that execute no job steps do not satisfy this boundary.
