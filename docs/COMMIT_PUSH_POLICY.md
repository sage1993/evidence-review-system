...
GitHub- or platform-generated merge commits are exempt because their subject is
generated by the merge platform. This exemption does not apply to a
contributor-authored commit on the issue branch.

A contributor-authored issue-branch commit subject that omits the issue or
change type is not eligible for issue acceptance until corrected in a new
commit.
...
merged, issue not `CLOSED`, nonconforming contributor-authored issue-branch
commit subject, ambiguous protection state, or unresolved security/provenance
concern. Do not bypass a repository control or an explicit maintainer-required
check.
