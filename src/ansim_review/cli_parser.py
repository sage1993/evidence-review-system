...
def build_parser() -> argparse.ArgumentParser:
    ...
    evidence_migrate = evidence_stages.add_parser(
        "migrate", help="copy an evidence schema v1 database to schema v2"
    )
    ...
    source_batch = subparsers.add_parser(
        "source-batch", help="validate and ingest arbitrary user-provided PDF sources"
    )
    source_stages = source_batch.add_subparsers(dest="source_stage", required=True)
    source_prepare = source_stages.add_parser(
        "prepare", help="validate a source batch and report parser and routing states"
    )
    ...
    source_ingest = source_stages.add_parser(
        "ingest", help="build a searchable evidence SQLite database from a source batch"
    )
    ...
    parser_command = subparsers.add_parser(
        "parser", help="validate parser warnings and reproducibility without rerunning parsing"
    )
    parser_stages = parser_command.add_subparsers(dest="parser_stage", required=True)
    ...
    parser_warnings = parser_stages.add_parser(
        "warnings", help="collect parser warnings into review evidence"
    )
    warning_actions = parser_warnings.add_subparsers(dest="parser_action", required=True)
    warning_collect = warning_actions.add_parser(
        "collect", help="write a warning report and stable review queue"
    )
    warning_collect.add_argument("--source-manifest", required=True, type=Path)
    warning_collect.add_argument("--parser-artifacts", required=True, type=Path)
    ...
    rules = subparsers.add_parser(
        "rules", help="run governed rule golden tests, activation, and scope selection"
    )
    rule_stages = rules.add_subparsers(dest="rules_stage", required=True)
    run_golden = rule_stages.add_parser(
        "run-golden", help="evaluate one strict rule golden fixture manifest"
    )
    ...
    select_rules = rule_stages.add_parser(
        "select", help="verify active authority and select exact-scope rules"
    )
    ...
    question_track_a = question_stages.add_parser(
        "submit-track-a", help="validate Track A before issuing the Track B handoff"
    )
    ...
    question_track_b = question_stages.add_parser(
        "submit-track-b", help="validate Track B then finalize the prepared review run"
    )
    ...
    review_prepare = review_stages.add_parser(
        "prepare", help="validate deterministic inputs and prepare Track A artifacts"
    )
    ...
    review_serve = review_stages.add_parser(
        "serve", help="open an existing finalized run through the protected loopback server"
    )
    ...
    review_import_decision = review_stages.add_parser(
        "import-decision",
        help="validate an archival decision envelope and append it to the run",
    )
    review_import_decision.add_argument("--workspace", required=True, type=Path)
    review_import_decision.add_argument("--run-id", required=True)
    review_import_decision.add_argument("--envelope", required=True, type=Path)
    return parser
