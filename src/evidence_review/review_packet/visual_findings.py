...
_LEVEL_RE = re.compile(r"(?:^|[^0-9])\d+\s*f(?:[^a-z]|$)|\b(?:el|gl)\s*[+\-]?\s*\d", re.I)
...
def _category(candidate: Mapping[str, object]) -> str:
    ...
    value = _text(candidate.get("display_value")) or _text(candidate.get("normalized_candidate")) or _text(candidate.get("raw_value"))
    folded = value.casefold()
    if _BUILDING_RE.search(value) or "BUILDING" in candidate_type:
        return "building_identity"
    if "㎡" in value or "M²" in value.upper() or "AREA" in candidate_type or any(term in value for term in ("전용면적", "공용면적", "발코니면적", "생활면적", "확장면적")):
        return "area"
    if _LEVEL_RE.search(value) or "LEVEL" in candidate_type or "ELEVATION" in candidate_type or "층수" in value or "계획고" in value:
        return "level"
    if any(term in folded for term in _SPACE_TERMS) or any(term in candidate_type for term in ("ROOM", "SPACE", "PROGRAM")):
        return "space_program"
    numeric_parts = [part.strip() for part in re.split(r"[;,/]", value) if part.strip()]
    if (
        "DIMENSION" in candidate_type
        or "WIDTH" in candidate_type
        or "SETBACK" in candidate_type
        or _DISTANCE_RE.search(value)
        or len(numeric_parts) >= 4 and sum(any(ch.isdigit() for ch in item) for item in numeric_parts) >= 4
    ):
        return "dimension"
...
def _claim_ids(candidate: Mapping[str, object], relation: str) -> tuple[str, ...]:
    result: list[str] = []
    for index, item in enumerate(_sequence(candidate.get("claims", []), "candidate.claims")):
...
def _status(candidates: Sequence[Mapping[str, object]]) -> str:
    ...
    statuses = {
        ...
        for status in _sequence(candidate.get("review_statuses", []), "candidate.review_statuses")
    }
...
def build_semantic_visual_findings(pages: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    """Group raw OCR/visual candidates into stable reviewer-facing semantic findings."""
    grouped: dict[tuple[str, tuple[str, ...], str], list[Mapping[str, object]]] = {}
    for page_index, raw_page in enumerate(pages):
        ...
        for candidate_index, raw_candidate in enumerate(_sequence(page.get("candidates", []), "page.candidates")):
            candidate = _mapping(raw_candidate, f"page.candidates[{candidate_index}]")
            issue_ids = tuple(sorted({str(item) for item in _sequence(candidate.get("issue_ids", []), "candidate.issue_ids") if str(item)}))
    ...
    for (asset_key, issue_ids, category), candidates in sorted(grouped.items()):
        candidate_ids = sorted({_text(item.get("candidate_id")) for item in candidates if _text(item.get("candidate_id"))})
        ...
        direct_claim_ids = sorted({claim_id for candidate in candidates for claim_id in _claim_ids(candidate, "direct")})
        related_claim_ids = sorted({claim_id for candidate in candidates for claim_id in _claim_ids(candidate, "related")})
        identity = {
            "asset_key": asset_key,
