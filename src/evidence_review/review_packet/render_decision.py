...
def render_decision_form(model: Mapping[str, object], *, initially_hidden: bool = False) -> str:
    ...
    return "".join(
        (
            ...
            '<div class="decision-panel-heading"><h2 id="decision-heading">사람의 결정</h2>',
            '<button type="button" class="primary-action" data-add-decision '
            'aria-expanded="false" aria-controls="decision-editor">새 결정 기록</button></div>',
            '<div class="persisted-decision" data-persisted-decision hidden>',
            '<h3 class="visually-hidden">기록된 사람의 결정</h3>',
            '<dl>',
            '<div><dt class="visually-hidden">결정</dt>'
            '<dd data-persisted-decision-value></dd></div>',
            '<div><dt>검토자</dt><dd data-persisted-reviewer></dd></div>',
            '<div><dt class="visually-hidden">검토 시각</dt>'
            '<dd data-persisted-reviewed-at></dd></div>',
            '</dl>',
            '<details class="decision-record-details"><summary>기록 보기</summary>',
            '<p data-persisted-notes></p>',
            ...
            '<div id="decision-editor" data-decision-editor hidden>',
            '<p>검토 결과를 선택하고 필요한 의견을 입력하십시오.</p>',
            ...
            '<p id="notes-help" class="visually-hidden">'
            '동의 이외의 결정을 선택하면 의견이 필요합니다.</p>',
            ...
            '<div class="decision-actions"><button type="button" class="secondary-action" '
            'data-cancel-decision>취소</button><button class="primary-action" data-protected-only ',
