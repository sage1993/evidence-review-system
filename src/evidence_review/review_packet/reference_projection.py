"""Project reference evidence into bounded presentation metadata."""
...
def _reference_type(
    ...
    combined = " ".join(
        (normalized_evidence, normalized_element, normalized_visual)
    )
    if "drawing" in combined:
        return "DRAWING"
    if "diagram" in combined:
        return "DIAGRAM"
    if normalized_evidence == "visual" or normalized_element in {
        "figure",
        "image",
    }:
        return "IMAGE"
...
def project_reference_record(
    *,
    ...
    """Return allowlisted display metadata without exposing raw parser data."""
    reference_type = _reference_type(evidence_type, element_type, visual_kind)
    table_source = (
        table_raw_json if table_raw_json is not None else element_raw_json
    )
    return {
        "type": reference_type,
