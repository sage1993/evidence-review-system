...
root.querySelector('[data-findings-toggle]')?.addEventListener('click',event=>{const button=event.currentTarget,collapsed=root.dataset.findingsCollapsed!=='true';root.dataset.findingsCollapsed=String(collapsed);button.setAttribute('aria-expanded',String(!collapsed));button.textContent=collapsed?'관찰 목록 펼치기':'관찰 목록 접기'});
