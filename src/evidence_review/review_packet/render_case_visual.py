...
.viewer-controls button{width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;padding:0;line-height:0}
.finding-filter{height:32px;padding:0 8px;font-size:12px}
...
CASE_VISUAL_SCRIPT = (
    CASE_VISUAL_SCRIPT.replace(
        ...
        "function reset(stage){states.set(stage,{scale:1,x:0,y:0,drag:false,px:0,py:0});apply(stage)}function fitScreen(stage){if(stage)reset(stage)}function fitWidth(stage){if(!stage)return;const page=pages.find(page=>page.contains(stage)),[pw,ph]=pageDimensions(page),rect=stage.getBoundingClientRect(),screen=Math.min(rect.width/pw,rect.height/ph),width=rect.width/pw;if(!(pw>0&&ph>0&&screen>0&&width>0))return;states.set(stage,{scale:Math.min(5,Math.max(.5,width/screen)),x:0,y:0,drag:false,px:0,py:0});apply(stage)}function originalSize(stage){if(!stage)return;const page=pages.find(page=>page.contains(stage)),[pw,ph]=pageDimensions(page),rect=stage.getBoundingClientRect(),screen=Math.min(rect.width/pw,rect.height/ph);if(!(pw>0&&ph>0&&screen>0))return;states.set(stage,{scale:Math.min(5,Math.max(.5,1/screen)),x:0,y:0,drag:false,px:0,py:0});apply(stage)}",
    ).replace(
