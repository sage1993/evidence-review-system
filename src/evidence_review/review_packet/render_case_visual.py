...
def render_case_visual_review(model: Mapping[str, object]) -> str:
    ...
    return "".join(
        (
            ...
            '<section id="case-visual-review" class="visual-review-workspace" data-findings-open="false" aria-label="기준 근거와 사용자 파일 대조 Workspace">',
            ...
            "".join(page_html),
            "</div></section></div>",
            '<button class="findings-toggle" type="button" data-findings-toggle aria-controls="case-findings" aria-expanded="false">대조 결과</button>',
            '<button class="findings-backdrop" type="button" data-findings-backdrop aria-label="대조 결과 닫기" hidden></button>',
            '<aside id="case-findings" class="findings-panel" aria-label="대조 결과"><header><strong>대조 결과</strong><span>Findings</span><button type="button" class="findings-close" data-findings-close aria-label="대조 결과 닫기">×</button></header><div class="findings-body">',
            "".join(finding_html),
...
.findings-toggle,.findings-close,.findings-backdrop{display:none}@media(max-width:1366px){.workspace-grid{grid-template-columns:minmax(0,1fr);position:relative}.findings-toggle{display:block;position:absolute;right:12px;top:8px;z-index:8;min-height:30px;padding:0 10px;border:1px solid var(--line);border-radius:6px;background:#fff}.findings-panel{position:absolute;right:0;top:0;bottom:0;width:min(340px,88vw);z-index:10;transform:translateX(100%);transition:transform .18s ease;box-shadow:-10px 0 28px rgba(16,24,40,.18)}[data-findings-open="true"] .findings-panel{transform:translateX(0)}.findings-backdrop{position:absolute;inset:0;z-index:9;border:0;background:rgba(16,24,40,.32)}[data-findings-open="true"] .findings-backdrop{display:block}.findings-close{display:grid;place-items:center;width:28px;height:28px;border:0;background:transparent;font-size:20px}.reference-toolbar,.subject-toolbar{padding-left:8px;padding-right:8px}.viewer-controls{gap:3px}}

"""


CASE_VISUAL_SCRIPT = r"""
{const legacyRoot=document.getElementById('case-visual-review');if(legacyRoot)legacyRoot.dataset.bound='1'}
...
(()=>{
const root=document.getElementById('case-visual-review');if(!root||root.dataset.visualBound==='1')return;root.dataset.visualBound='1';
const referencePages=[...root.querySelectorAll('[data-reference-page]')],subjectPages=[...root.querySelectorAll('[data-case-page]')],findings=[...root.querySelectorAll('[data-case-finding]')],references=[...root.querySelectorAll('[data-case-reference]')],referenceOverlays=[...root.querySelectorAll('[data-reference-anchor]')],subjectOverlays=[...root.querySelectorAll('[data-case-overlay]')];
const states=new WeakMap();let referencePageIndex=0,subjectPageIndex=0,findingIndex=0;const referencePageNumber=root.querySelector('[data-reference-page-number]'),subjectPageNumber=root.querySelector('[data-case-page-number]'),referenceZoom=root.querySelector('[data-reference-zoom]'),subjectZoom=root.querySelector('[data-case-zoom]'),findingNumber=root.querySelector('[data-finding-number]');
function stageState(stage){let value=states.get(stage);if(!value){value={scale:1,x:0,y:0,drag:false,px:0,py:0};states.set(stage,value)}return value}function stageTransform(stage){return stage.querySelector('[data-reference-transform],[data-case-transform]')}function applyStage(stage){const value=stageState(stage),transform=stageTransform(stage),label=stage.matches('[data-reference-stage]')?referenceZoom:subjectZoom;if(transform)transform.style.transform=`translate(${value.x}px,${value.y}px) scale(${value.scale})`;if(label)label.textContent=`${Math.round(value.scale*100)}%`}
function fitStage(stage){states.set(stage,{scale:1,x:0,y:0,drag:false,px:0,py:0});applyStage(stage)}
function focusStage(stage,shape){if(!stage||!shape)return;const rect=stage.getBoundingClientRect(),x=Number(shape.getAttribute('x')||shape.getAttribute('cx')||0),y=Number(shape.getAttribute('y')||shape.getAttribute('cy')||0),width=Math.max(1,Number(shape.getAttribute('width')||16)),height=Math.max(1,Number(shape.getAttribute('height')||16)),box=stage.querySelector('svg')?.viewBox?.baseVal;if(!box||!rect.width||!rect.height)return;const scale=Math.min(5,Math.max(.5,Math.min(box.width/(width*2),box.height/(height*2)))),value=stageState(stage);value.scale=scale;value.x=rect.width/2-(x+width/2)/box.width*rect.width*scale;value.y=rect.height/2-(y+height/2)/box.height*rect.height*scale;applyStage(stage)}
function showReferencePage(key){const index=referencePages.findIndex(page=>page.dataset.referencePage===key);if(index<0)return null;referencePageIndex=index;referencePages.forEach((page,item)=>{page.hidden=item!==index;page.classList.toggle('is-active',item===index)});if(referencePageNumber)referencePageNumber.textContent=String(index+1);const stage=referencePages[index].querySelector('[data-reference-stage]');if(stage)applyStage(stage);return stage}
function showSubjectPage(key){const index=subjectPages.findIndex(page=>page.dataset.casePage===key);if(index<0)return null;subjectPageIndex=index;subjectPages.forEach((page,item)=>{page.hidden=item!==index;page.classList.toggle('is-active',item===index)});if(subjectPageNumber)subjectPageNumber.textContent=String(index+1);const stage=subjectPages[index].querySelector('[data-case-stage]');if(stage)applyStage(stage);return stage}
function activateFinding(index){if(!findings.length)return;findingIndex=(index+findings.length)%findings.length;const active=findings[findingIndex],id=active.dataset.caseFinding||'';findings.forEach((finding,item)=>{finding.hidden=item!==findingIndex;finding.classList.toggle('is-active',item===findingIndex)});references.forEach(reference=>{const selected=reference.dataset.caseReference===id;reference.hidden=!selected;reference.classList.toggle('is-active',selected)});subjectOverlays.forEach(overlay=>overlay.classList.toggle('is-active',overlay.dataset.caseOverlay===id));const reference=references.find(item=>item.dataset.caseReference===id),detail=reference?.querySelector('[data-reference-anchor-detail]'),anchorId=detail?.dataset.referenceAnchorDetail||'',referenceStage=detail?showReferencePage(detail.dataset.assetKey||''):null,referenceOverlay=referenceOverlays.find(item=>item.dataset.referenceAnchor===anchorId),subjectStage=showSubjectPage(active.dataset.casePageKey||''),subjectOverlay=subjectOverlays.find(item=>item.dataset.caseOverlay===id);referenceOverlays.forEach(overlay=>overlay.classList.toggle('is-active',overlay===referenceOverlay));if(findingNumber)findingNumber.textContent=String(findingIndex+1);requestAnimationFrame(()=>{if(referenceStage&&referenceOverlay)focusStage(referenceStage,referenceOverlay.querySelector('.reference-anchor-shape'));if(subjectStage&&subjectOverlay)focusStage(subjectStage,subjectOverlay.querySelector('.case-visual-shape'))})}
function zoomStage(stage,factor,clientX,clientY){if(!stage)return;const value=stageState(stage),rect=stage.getBoundingClientRect(),cx=clientX??rect.left+rect.width/2,cy=clientY??rect.top+rect.height/2,lx=(cx-rect.left-value.x)/value.scale,ly=(cy-rect.top-value.y)/value.scale,next=Math.min(5,Math.max(.5,value.scale*factor));value.x=cx-rect.left-lx*next;value.y=cy-rect.top-ly*next;value.scale=next;applyStage(stage)}
function bindStage(stage){stage.addEventListener('wheel',event=>{event.preventDefault();zoomStage(stage,event.deltaY<0?1.12:.89,event.clientX,event.clientY)},{passive:false});stage.addEventListener('dblclick',()=>fitStage(stage));stage.addEventListener('pointerdown',event=>{if(event.button!==0)return;const value=stageState(stage);value.drag=true;value.px=event.clientX;value.py=event.clientY;stage.setPointerCapture(event.pointerId);stage.classList.add('is-dragging')});stage.addEventListener('pointermove',event=>{const value=stageState(stage);if(!value.drag)return;value.x+=event.clientX-value.px;value.y+=event.clientY-value.py;value.px=event.clientX;value.py=event.clientY;applyStage(stage)});const stop=event=>{stageState(stage).drag=false;stage.classList.remove('is-dragging');try{stage.releasePointerCapture(event.pointerId)}catch(_){}};stage.addEventListener('pointerup',stop);stage.addEventListener('pointercancel',stop)}
referencePages.forEach(page=>{const stage=page.querySelector('[data-reference-stage]');if(stage)bindStage(stage)});subjectPages.forEach(page=>{const stage=page.querySelector('[data-case-stage]');if(stage)bindStage(stage)});
function pageButton(selector,pages,getIndex,key,show,delta){root.querySelector(selector)?.addEventListener('click',()=>{const index=(getIndex()+delta+pages.length)%pages.length;show(pages[index]?.dataset[key]||'')})}pageButton('[data-reference-prev]',referencePages,()=>referencePageIndex,'referencePage',showReferencePage,-1);pageButton('[data-reference-next]',referencePages,()=>referencePageIndex,'referencePage',showReferencePage,1);pageButton('[data-case-prev]',subjectPages,()=>subjectPageIndex,'casePage',showSubjectPage,-1);pageButton('[data-case-next]',subjectPages,()=>subjectPageIndex,'casePage',showSubjectPage,1);
root.querySelector('[data-reference-zoom-in]')?.addEventListener('click',()=>zoomStage(referencePages[referencePageIndex]?.querySelector('[data-reference-stage]'),1.2));root.querySelector('[data-reference-zoom-out]')?.addEventListener('click',()=>zoomStage(referencePages[referencePageIndex]?.querySelector('[data-reference-stage]'),.8333));root.querySelector('[data-reference-fit]')?.addEventListener('click',()=>{const stage=referencePages[referencePageIndex]?.querySelector('[data-reference-stage]');if(stage)fitStage(stage)});
root.querySelector('[data-case-zoom-in]')?.addEventListener('click',()=>zoomStage(subjectPages[subjectPageIndex]?.querySelector('[data-case-stage]'),1.2));root.querySelector('[data-case-zoom-out]')?.addEventListener('click',()=>zoomStage(subjectPages[subjectPageIndex]?.querySelector('[data-case-stage]'),.8333));root.querySelector('[data-case-reset]')?.addEventListener('click',()=>{const stage=subjectPages[subjectPageIndex]?.querySelector('[data-case-stage]');if(stage)fitStage(stage)});
root.querySelector('[data-finding-prev]')?.addEventListener('click',()=>activateFinding(findingIndex-1));root.querySelector('[data-finding-next]')?.addEventListener('click',()=>activateFinding(findingIndex+1));
const split=root.querySelector('[data-case-split]'),divider=root.querySelector('[data-case-divider]');function setSplit(percent){if(!split||!divider)return;const value=Math.min(70,Math.max(26,percent));split.style.setProperty('--reference-width',`${value}%`);divider.setAttribute('aria-valuenow',String(Math.round(value)))}if(split&&divider){let dragging=false;divider.addEventListener('pointerdown',event=>{if(event.button!==0)return;dragging=true;divider.setPointerCapture(event.pointerId)});divider.addEventListener('pointermove',event=>{if(!dragging)return;const rect=split.getBoundingClientRect();setSplit((event.clientX-rect.left)/rect.width*100)});const stop=event=>{dragging=false;try{divider.releasePointerCapture(event.pointerId)}catch(_){}};divider.addEventListener('pointerup',stop);divider.addEventListener('pointercancel',stop);divider.addEventListener('dblclick',()=>setSplit(42));divider.addEventListener('keydown',event=>{const now=Number(divider.getAttribute('aria-valuenow')||42);if(event.key==='ArrowLeft'){event.preventDefault();setSplit(now-2)}if(event.key==='ArrowRight'){event.preventDefault();setSplit(now+2)}if(event.key==='Home'){event.preventDefault();setSplit(42)}})}
const findingsToggle=root.querySelector('[data-findings-toggle]'),findingsClose=root.querySelector('[data-findings-close]'),findingsBackdrop=root.querySelector('[data-findings-backdrop]');
function setFindingsOpen(open,returnFocus=false){root.dataset.findingsOpen=String(open);findingsToggle?.setAttribute('aria-expanded',String(open));if(findingsBackdrop)findingsBackdrop.hidden=!open;if(!open&&returnFocus)findingsToggle?.focus()}
findingsToggle?.addEventListener('click',()=>setFindingsOpen(true));findingsClose?.addEventListener('click',()=>setFindingsOpen(false,true));findingsBackdrop?.addEventListener('click',()=>setFindingsOpen(false,true));document.addEventListener?.('keydown',event=>{if(event.key==='Escape'&&root.dataset.findingsOpen==='true'){event.preventDefault();setFindingsOpen(false,true)}});

if(findings.length)activateFinding(0);else{showReferencePage(referencePages[0]?.dataset.referencePage||'');showSubjectPage(subjectPages[0]?.dataset.casePage||'')}
})();
"""
