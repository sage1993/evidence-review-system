...
def _reference_anchor_card(
    ...
    return (
        ...
        f"{_reference_raster_svg(page)}{content}{_reference_anchor_overlay(anchor, page)}"
        "</div></article>"
    )
...
.reference-raster-layer,.reference-overlay-layer{position:absolute;inset:0;width:100%;height:100%;pointer-events:none}
"""
...
CASE_VISUAL_SCRIPT = (
    CASE_VISUAL_SCRIPT.replace(
        ...
        "function ensureReferenceRaster(scope){if(!scope)return;scope.querySelectorAll('[data-reference-page-image]').forEach(image=>{if(image.getAttribute('href'))return;const src=image.dataset.referencePageSrc||'';if(src)image.setAttribute('href',src)})}function activateCard(index,{focus=true}={}){",
    ).replace(
        "ref.classList.toggle('is-active',on);if(on){const direct=",
        "ref.classList.toggle('is-active',on);if(on){ensureReferenceRaster(ref);const direct=",
    )
)
