...
# Findings are an issue list, not a paginated result feed.  Keep filtering and
# focus synchronization, while ensuring every issue remains reachable in one view.
CASE_VISUAL_SCRIPT = CASE_VISUAL_SCRIPT.replace(
    "function renderResultPage({activate=true}={}){const indexes=filteredCardIndexes(),total=Math.max(1,Math.ceil(indexes.length/PAGE_SIZE));resultPage=Math.max(0,Math.min(resultPage,total-1));const slice=new Set(indexes.slice(resultPage*PAGE_SIZE,(resultPage+1)*PAGE_SIZE));cards.forEach((card,index)=>{card.hidden=!slice.has(index)});if(resultPageNo)resultPageNo.textContent=String(resultPage+1);if(resultPageTotal)resultPageTotal.textContent=String(total);if(activate&&indexes.length){const preferred=slice.has(activeCardIndex)?activeCardIndex:indexes[resultPage*PAGE_SIZE];activateCard(preferred,{focus:true})}}",
    "function renderResultPage({activate=true}={}){const indexes=filteredCardIndexes();cards.forEach((card,index)=>{card.hidden=!indexes.includes(index)});if(activate&&indexes.length){const preferred=indexes.includes(activeCardIndex)?activeCardIndex:indexes[0];activateCard(preferred,{focus:true})}}",
).replace(
    "renderResultPage({activate:false});",
    "cards.forEach(card=>{card.hidden=false});",
).replace(
    "root.querySelector('[data-finding-prev]')?.addEventListener('click',()=>{resultPage--;renderResultPage({activate:true})});root.querySelector('[data-finding-next]')?.addEventListener('click',()=>{resultPage++;renderResultPage({activate:true})});",
    "",
).replace(
    "data-case-decision-open",
    "data-case-decision-trigger",
).replace(
    "data-case-decision-backdrop",
    "data-case-decision-surface",
)
