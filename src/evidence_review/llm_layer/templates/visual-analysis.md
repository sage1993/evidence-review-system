  ...
  "observations": [
    {
      ...
      "candidate_type": "<neutral visual feature type>",
      "geometry": {
...
- Use a tight BBOX/POLYGON/LINESTRING/POINT around the visible feature. Do not create placeholder geometry.
- `raw_value` may reproduce visible text or a visible label. If text is unreadable, use null rather than guessing.
- `normalized_candidate` may normalize an actually visible value, but must not add a value that is not visible.
- `candidate_type` describes a neutral visual feature, not a legal conclusion.
