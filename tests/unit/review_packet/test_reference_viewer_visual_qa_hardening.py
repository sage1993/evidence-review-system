...
def test_visual_review_adds_true_4k_scale_tier() -> None:
    ...
    assert ".finding-card h3{font-size:18px;margin:10px 0 9px}" in html
    assert ".comparison-grid dd{font-size:15px;line-height:1.45}" in html
    assert ".reference-empty strong{font-size:17px}" in html
    assert ".reference-empty p{max-width:440px;line-height:1.6}" in html
    assert ".decision-open{height:48px;padding:0 16px;font-size:14px}" in html
