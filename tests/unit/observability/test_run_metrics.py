...
def test_stage_events_are_append_only_and_aggregate_attempts(tmp_path: Path) -> None:
    run = tmp_path / "runs" / "RUN-0123456789ABCDEF0123"
    run.mkdir(parents=True)

    append_stage(run, make_stage("retrieval", _at(0), _at(1), duration_ms=1000))
    append_stage(
        run,
        make_stage(
            "retrieval",
    ...
    assert metrics["stages"][1]["reason_code"] == "SQLITE_BUSY"


def test_external_wait_is_reported_but_excluded_from_deterministic_total(tmp_path: Path) -> None:
    run = tmp_path / "runs" / "RUN-0123456789ABCDEF0123"
    run.mkdir(parents=True)
