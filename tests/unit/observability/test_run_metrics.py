...
def test_completed_browser_handoff_freezes_formal_metrics_but_keeps_events(
    tmp_path: Path,
) -> None:
    run = tmp_path / "runs" / "RUN-0123456789ABCDEF0123"
    run.mkdir(parents=True)
    append_stage(
        run,
        make_stage("protected-server-start", _at(0), _at(1), duration_ms=321),
    )
    append_stage(
        run,
        make_stage("browser-dispatch", _at(1), _at(2), duration_ms=76),
    )
    formal = load_run_metrics(run)

    append_stage(
        run,
        make_stage("protected-server-start", _at(3), _at(4), duration_ms=161),
    )
    append_stage(
        run,
        make_stage("browser-dispatch", _at(4), _at(5), duration_ms=74),
    )
    ...
    metrics = load_run_metrics(run)
    assert metrics == formal
    assert metrics["deterministic_total_ms"] == 397
    assert [stage["name"] for stage in metrics["stages"]] == [
        "protected-server-start",
        "browser-dispatch",
    ]
    assert json.loads((run / "run-metrics.json").read_text(encoding="utf-8")) == metrics
