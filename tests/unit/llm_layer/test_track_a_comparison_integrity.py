...
from evidence_review.llm_layer.track_a import ClaimReferences, EvidenceExcerpt, TrackABundle, ValidatedTrackA
from evidence_review.llm_layer.validators import validate_track_a_integrity
from evidence_review.rule_engine.fact_rule_comparison import compare_fact_to_threshold, comparison_document
...
def _bundle(comparison: dict[str, object], issue_ids: tuple[str, ...] = ("I2",)) -> TrackABundle:
    return TrackABundle(
        ...
        evidence=(EvidenceExcerpt(_citation(), "승강장 경계로부터 250미터 이내를 원칙으로 한다.", issue_ids, "rule"),),
        rules=(), calculations=(), approved_rule_result_ids=(),
    )


def _validated(issue_id: str = "I2") -> ValidatedTrackA:
    claim = Claim("CL-1", "300미터는 일반 기준 250미터를 초과한다.", ("CIT-E1",), ("300", "250"), (issue_id,))
