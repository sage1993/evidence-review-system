...
def _snapshot() -> EvidenceSnapshot:
    clauses = (
        ...
        (
            "C-PARKING-PRIVATE",
            "공공지원민간임대주택 주차기준",
            ("임대형기숙사를 제외한 안심주택의 주차장 설치기준은 "
             "해당 주택 유형에 적용되는 기준을 따른다."),
        ),
        (
            "C-PARKING-DORM",
            "임대형기숙사 주차기준",
            ("임대형기숙사와 다른 주택용도를 복합하는 경우 "
             "주차장 설치기준은 각 주택용도별 기준을 각각 적용한다."),
        ),
        (
            "C-FAR",
            "준공업지역 용적률",
        ...
        (
            "C-INDUSTRIAL",
            "산업부지 확보비율",
            ("준공업지역에서는 산업부지 확보비율을 적용하며 "
             "통합심의 결과에 따라 확보비율을 완화할 수 있다."),
        ),
        (
            "C-DUP",
            "지구단위계획 주차완화",
...
def test_fallback_stops_after_first_successful_stage(tmp_path: Path) -> None:
    ...
    assert result.success_stage == FallbackStage.EXACT_CLAUSE
