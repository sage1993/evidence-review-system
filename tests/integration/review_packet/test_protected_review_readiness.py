...
def _case_model(image_sha256: str) -> dict[str, object]:
    return {
        "run_id": RUN_ID,
...
def _write_workspace(root: Path) -> str:
    ...
    for attachment_id, page_number in (
        (UNTILED_ATTACHMENT, 1),
        (TILED_ATTACHMENT, 2),
    ):
        page_dir = root / "case-page-images-hq-v1" / attachment_id
        page_dir.mkdir(parents=True)
        (page_dir / f"page-{page_number:04d}.png").write_bytes(VALID_MINIMAL_PNG)

    tile_dir = root / "case-page-tiles-v1" / TILED_ATTACHMENT / "page-0002"
    tile_dir.mkdir(parents=True)
    tile_name = "tile-0000-0000.png"
    (tile_dir / tile_name).write_bytes(VALID_MINIMAL_PNG)
...
def test_real_server_protected_readiness_decodes_and_hashes_tiled_and_untiled_case(
    ...
    expected_sha256 = _write_workspace(tmp_path)
